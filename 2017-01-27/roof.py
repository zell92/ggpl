...
def createRoof(vertexes,angle,diagonal):
	"""
	...
	for i in range(len(directions)):
		if i == 0:
			f = MKPOL([[[falde[i][0][0],falde[i][0][1],0],[falde[i][1][0],falde[i][1][1],0],[intersezioni[i][0],intersezioni[i][1],falde[1][2][2]],[intersezioni[len(directions)-1][0],intersezioni[len(directions)-1][1],falde[0][2][2]]],[[4,3,2,1]],None])

		else:
			f = MKPOL([[[falde[i][0][0],falde[i][0][1],0],[falde[i][1][0],falde[i][1][1],0],[intersezioni[i][0],intersezioni[i][1],falde[1][2][2]],[intersezioni[i-1][0],intersezioni[i-1][1],falde[0][2][2]]],[[4,3,2,1]],None])
	...
	tetto=TEXTURE(["texture/tegole.jpg", TRUE, FALSE, 1, 1, 0, 3, 3])(tetto)
...
def main():
	...
	ver=[[0,2],[2,3],[3,5],[5,4],[6,1.5],[4,.5],[3.5,1.5],[1,0]]
	#ver=[[0,0],[1,100],[100,100],[90,0]]
	altezza=getMinDistPitch(ver)/2
	VIEW(createRoof(ver,PI/5,altezza))
